...
1);if(g>d)break}}
function drawToggleButtons(){let a=document.getElementById("toggle-buttons-header");a.innerHTML="";for(const b of categoriesAll)button=document.createElement("button"),button.classList.add("toggle-buttons"),button.id=b,button.title="Kalender ein- bzw. ausschalten",button.textContent=b,categoriesSelected.includes(b)?(button.classList.remove("button-switched-off"),button.classList.add("button-switched-on"),button.style.backgroundColor=categoryColors[b]):(button.classList.remove("button-switched-on"),button.classList.add("button-switched-off")),
a.appendChild(button),document.getElementById(b).addEventListener("click",function(){toggleCategory(b)});button=document.createElement("button");buttonImage=document.createElement("img");weekview?(buttonImage.src=svgFiles.month,button.title="zur Monatsansicht wechseln"):(buttonImage.src=svgFiles.week,button.title="zur Wochenansicht wechseln");buttonImage.classList.add("svg-icon");button.classList.add("toggle-buttons");button.classList.add("setting-buttons");button.appendChild(buttonImage);button.addEventListener("click",
function(){toggleWeekMonth()});a.appendChild(button);button=document.createElement("button");buttonImage=document.createElement("img");buttonImage.classList.add("svg-icon");buttonImage.src=svgFiles.save;button.classList.add("toggle-buttons","setting-buttons");button.id="save";button.title="kopiere alle Anzeigeeinstellungen in die URL\n(z.B. um sie als Lesezeichen im Browser zu speichern)";button.appendChild(buttonImage);button.addEventListener("click",function(){addParamsToURL()});a.appendChild(button);
button=document.createElement("button");buttonImage=document.createElement("img");buttonImage.src=svgFiles.question;buttonImage.classList.add("svg-icon");button.classList.add("toggle-buttons");button.classList.add("setting-buttons");button.id="save";button.title="Hilfe anzeigen";button.appendChild(buttonImage);button.addEventListener("click",function(){showHelp()});a.appendChild(button)}
