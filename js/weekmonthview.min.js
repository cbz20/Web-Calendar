...
async function parseICS(a){for(const c of categoriesAll){var b=await fetch(a[c]);if(!b.ok)throw Error(`Response status: ${b.status}`);b=await b.text();b=ICAL.parse(b);b=(new ICAL.Component(b)).getAllSubcomponents("vevent").map(d=>parse_event(d,c));b=b.filter(d=>0<Object.keys(d).length);events.push(...b)}}
...
1);if(g>d)break}}
function drawToggleButtons(){let a=document.getElementById("toggle-buttons-header");a.innerHTML="";for(const b of categoriesAll)button=document.createElement("button"),button.classList.add("toggle-buttons"),button.id=b,button.title="Kalender ein- bzw. ausschalten",button.textContent=b,categoriesSelected.includes(b)?(button.classList.remove("button-switched-off"),button.style.backgroundColor=categoryColors[b]):button.classList.add("button-switched-off"),a.appendChild(button),document.getElementById(b).addEventListener("click",function(){toggleCategory(b)});
button=document.createElement("button");buttonImage=document.createElement("img");weekview?(buttonImage.src=svgFiles.month,button.title="zur Monatsansicht wechseln"):(buttonImage.src=svgFiles.week,button.title="zur Wochenansicht wechseln");buttonImage.classList.add("svg-icon");button.classList.add("toggle-buttons");button.classList.add("setting-buttons");button.appendChild(buttonImage);button.addEventListener("click",function(){toggleWeekMonth()});a.appendChild(button);button=document.createElement("button");
buttonImage=document.createElement("img");buttonImage.classList.add("svg-icon");buttonImage.src=svgFiles.save;button.classList.add("toggle-buttons","setting-buttons");button.id="save";button.title="kopiere alle Anzeigeeinstellungen in die URL\n(z.B. um sie als Lesezeichen im Browser zu speichern)";button.appendChild(buttonImage);button.addEventListener("click",function(){addParamsToURL()});a.appendChild(button);button=document.createElement("button");buttonImage=document.createElement("img");buttonImage.src=
svgFiles.question;buttonImage.classList.add("svg-icon");button.classList.add("toggle-buttons");button.classList.add("setting-buttons");button.id="save";button.title="Hilfe anzeigen";button.appendChild(buttonImage);button.addEventListener("click",function(){showHelp()});a.appendChild(button)}
...
function showHelp(a){document.body.classList.add("no-scroll");document.getElementById("modal-title").textContent="Hilfe zur Benutzung des Fakult\u00e4tskalenders";a="<li>\n    <b>alle Kalender:</b> \n    <a href='https://mail.ruhr-uni-bochum.de/SOGo/dav/public/termixxd/Calendar/'>https://mail.ruhr-uni-bochum.de/SOGo/dav/public/termixxd/Calendar/</a>\n    </li>";for(var b of categoriesAll)a+="<li><b>",a+=b,a+=":</b> <a href='",a+=links[b],a+="'>",a+=links[b],a+="</a></li>";b="";for(const c of categoriesAll)b+=
"<li><b>",b+=c,b+=":</b> <a href='",b+=links[c].replace(/public\//g,"").replace(/.ics/g,"/"),b+="'>",b+=links[c].replace(/public\//g,"").replace(/.ics/g,"/"),b+="</a></li>";document.getElementById("modal-body").innerHTML='\n\t        <div class="helpsection">\n\t        Zwischen Wochen- und Monatsansicht wechseln\n\t\t</div>\n\t\t<p> \n\t\tKlicken Sie dazu die Schalter \n\t\t<button class=\'toggle-buttons setting-buttons dummy-button\' disabled>\n\t\t<img src="'+svgFiles.week+'" class="svg-icon">\n\t\t</button> \n\t\tbzw.\n\t\t<button class=\'toggle-buttons setting-buttons dummy-button\' disabled>\n\t\t<img src="'+
